...
class Extractor:
    ...
    def get_dataset(self):
        return load_dataset(args, self.config, self.tokenizer, split="test")
    @torch.no_grad()
    def extract(self):
        """
...
if __name__ == "__main__":
    ...
    extractor = Extractor()
    input = extractor.get_dataset()[0]
    input_ids = input['src_input_ids']
    print(input)
    print(extractor.tokenizer.decode(input['trg_input_ids']))
    print(extractor.tokenizer.decode(input_ids))
    count = ""
    for span in input['candidate_kp_masks']: 
        for start, end in span:
            count += extractor.tokenizer.decode(input_ids[start:end]) + "; "
    print(count)
    # if args.do_generate:
