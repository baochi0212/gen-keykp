...
if not os.path.exists(f"./llm_valid_results/output_{model_type}"):
    os.mkdir(f"./llm_valid_results/output_{model_type}")
...
for dataset in ["kp20k"]:
    print("Infer the dataset: ", dataset)
# for dataset in ["inspec"]:
    present_writer = open(f"./llm_valid_results/output_{model_type}/{model_type}_{dataset}_pre.txt", "w")
    abs_writer = open(f"./llm_valid_results/output_{model_type}/{model_type}_{dataset}_abs.txt", "w")
    passed_line = len(open(f"./llm_valid_results/output_{model_type}/{model_type}_{dataset}_pre.txt", "r").readlines())
    print("Passed", passed_line)
    documents = []
    for i, line in tqdm(enumerate(open(f"./data/{dataset}/valid_src_filtered_bart.txt").readlines())):
        if i < passed_line:
            continue
        document = line.replace("<eos>", "").strip()
        documents.append(document)
    
    chats = make_finetune_prompt(documents=documents[:500])
