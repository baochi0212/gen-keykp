...
if __name__ == "__main__":
    ...
    step = args.step
    # global variables
    src_filepath = os.path.join(args.data_dir, f"test_src_filtered_bart.txt")
    trg_filepath = os.path.join(args.data_dir, f"test_trg_filtered_bart.txt")
    dataset_name = os.path.basename(args.data_dir)
    pre_pred_filepath = os.path.join(args.test_output_dir)
    abs_pred_filepath = os.path.join(args.test_output_dir2)
    if not os.path.exists(f"./llm_results/result_{args.model_type}"):
        os.mkdir(f"./llm_results/result_{args.model_type}")
    res_filepath = f"./llm_results/result_{args.model_type}/results_{dataset_name}_step_{step}.txt"
